...
public class ArmorBehavior : CollectibleBehavior, IArmor, IModularArmor, IAffectsPlayerStats
{
    ...
    public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
    {
        ...
        DamageResistData displayResists = QuenchableStatUtil.ApplyArmorProtection(inSlot.Itemstack, Resists);

        if (displayResists.Resists.Values.Any(value => value != 0))
        {
            dsc.AppendLine(Lang.Get("combatoverhaul:armor-fraction-protection"));
            foreach ((EnumDamageType type, float level) in displayResists.Resists.Where(entry => entry.Value > 0))
            {
        ...
        if (displayResists.FlatDamageReduction.Values.Any(value => value != 0))
        {
            dsc.AppendLine(Lang.Get("combatoverhaul:armor-flat-protection"));
            foreach ((EnumDamageType type, float level) in displayResists.FlatDamageReduction.Where(entry => entry.Value > 0))
            {
                string damageType = Lang.Get($"combatoverhaul:damage-type-{type}");
                dsc.AppendLine($"  {damageType}: {level}");
            }
        ...
        {
            Dictionary<EnumDamageType, float> resists = QuenchableStatUtil.ApplyArmorProtection(inSlot.Itemstack, ResistsByType.Values.First()).Resists;
            dsc.AppendLine(Lang.Get("combatoverhaul:armor-fraction-protection"));
        ...
        if (Stats.Values.Any(value => value != 0))
        {
            ...
            foreach ((string stat, float value) in Stats)
            {
                float shownValue = IsPenaltyStat(stat) ? value * penaltyMultiplier : value;
                if (shownValue != 0f) dsc.AppendLine($"  {Lang.Get($"combatoverhaul:stat-{stat}")}: {shownValue * 100:F1}%");
            }
        }
    ...
    public DamageResistData GetResists(ItemSlot slot, ArmorType type)
    {
        ...
        {
            return QuenchableStatUtil.ApplyArmorProtection(slot.Itemstack, ResistsByType[type]);
        }

        return QuenchableStatUtil.ApplyArmorProtection(slot.Itemstack, Resists);
    }
