...
public class ItemWearableArmor : Item, IWearableStatsSupplier
{
    ...
    {
        Dictionary<string, MultiTextureMeshRef>? meshrefs = ObjectCacheUtil.TryGet<Dictionary<string, MultiTextureMeshRef>>(api, "wearableArmorMeshRefs");
        if (meshrefs?.Count > 0)
        {
            ...
            ObjectCacheUtil.Delete(api, "wearableArmorMeshRefs");
        }
    ...
    public override void OnBeforeRender(ICoreClientAPI capi, ItemStack itemstack, EnumItemRenderTarget target, ref ItemRenderInfo renderinfo)
    {
        ...
        Dictionary<string, MultiTextureMeshRef> meshrefs = ObjectCacheUtil.GetOrCreate(capi, "wearableArmorMeshRefs", () => new Dictionary<string, MultiTextureMeshRef>());
    ...
    protected virtual string GetMeshCacheKey(ItemStack itemstack)
    {
        return "wearableArmorModelRef-" + itemstack.Collectible.Code;
    }
    protected virtual MeshData? GenMesh(ICoreClientAPI capi, ItemStack itemstack, ITexPositionSource texSource)
    {
        ...
        {
            AssetLocation invShapePath = new("shapes/" + attrObj["wearableInvShape"] + ".json");
            Shape invShape = Vintagestory.API.Common.Shape.TryGet(capi, invShapePath);
            if (invShape == null) return null;

            capi.Tesselator.TesselateShape(itemstack.Collectible, invShape, out MeshData invMesh);
            return invMesh;
        }

        CompositeShape? compArmorShape = attrObj["attachShape"].Exists
            ? attrObj["attachShape"].AsObject<CompositeShape>(null, itemstack.Collectible.Code.Domain)
            : (itemstack.Class == EnumItemClass.Item ? itemstack.Item.Shape : itemstack.Block.Shape);
        ...
        capi.Tesselator.TesselateShapeWithJointIds("entity", newShape, out MeshData meshdata, texSource, new Vec3f());
    ...
    protected virtual int GetRepairMatTypeCount(ItemSlot[] slots)
    {
        ...
        foreach (ItemSlot slot in slots)
        {
            ...
            bool found = false;
            if (IsWearableStack(slot)) continue;
    ...
    protected virtual int GetOrigMatCount(ItemSlot[] inSlots, ItemSlot outputSlot)
    {
        ...
        {
            if (slot.Empty || IsWearableStack(slot)) continue;
