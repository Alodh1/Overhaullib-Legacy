...
public class ArmorInventory : InventoryCharacter
{
    public ArmorInventory(string className, string playerUID, ICoreAPI api) : base(className, playerUID, api)
    {
        ...
        _disableVanillaArmorSlots = _api.ModLoader.IsModEnabled("combatoverhaul") || _api.ModLoader.IsModEnabled("combatoverhaulfork");
    ...
    public override void OnItemSlotModified(ItemSlot slot)
    {
        ...
        if (slot is GearSlot gearSlotModified)
        {
            try
            {
                RefreshGearSlots(dropDisabledContents: true);
            }
    ...
    internal void RefreshGearSlots(bool dropDisabledContents = false)
    {
        foreach (GearSlot gearSlot in _slots.OfType<GearSlot>())
        {
            gearSlot.CheckParentSlot();

            if (dropDisabledContents && !gearSlot.Enabled && !gearSlot.Empty)
            {
    ...
    private void ReloadBagInventory()
    {
        ...
        if (backpack is InventoryPlayerBackPacksCombatOverhaul combatOverhaulBackpack)
        {
            combatOverhaulBackpack.ReloadBagInventory();
            return;
        }
