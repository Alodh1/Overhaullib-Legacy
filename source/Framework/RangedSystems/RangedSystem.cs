...
public class RangedWeaponSystemServer
{
    ...
    {
        ItemSlot weaponSlot = GetWeaponSlot(player, packet.RightHand, packet.ItemId);
    ...
    private static ItemSlot GetWeaponSlot(IServerPlayer player, bool rightHand, int itemId)
    {
        if (!rightHand) return player.Entity.LeftHandItemSlot;

        ItemSlot active = player.Entity.ActiveHandItemSlot;
        if (active.Itemstack?.Item?.Id == itemId) return active;

        ItemSlot right = player.Entity.RightHandItemSlot;
        if (right.Itemstack?.Item?.Id == itemId) return right;

        return active.Itemstack != null ? active : right;
    }
