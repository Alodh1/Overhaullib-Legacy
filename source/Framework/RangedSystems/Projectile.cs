    ...
    protected override void applyCollision(EntityPos pos, float dtFactor)
    {
        ...
        if (collided)
        {
            ...
            if ((angleDeg > 90 - Config.MaxRicochetAngleDeg) && (angleDeg < 90 + Config.MaxRicochetAngleDeg) && (pos.Motion.Length() > Config.MinSpeedToRicochet))
            {
                ...
                AssetLocation soundLoc = (block?.Sounds?.Hit ?? block?.Sounds?.ByTool?.Values?.FirstOrDefault()?.Hit ?? block?.Sounds?.Break)?.Location
                    ?? new AssetLocation("game:sounds/player/destruct");
                entity.Api.World.PlaySoundAt(soundLoc, intersection.X, intersection.Y, intersection.Z);
