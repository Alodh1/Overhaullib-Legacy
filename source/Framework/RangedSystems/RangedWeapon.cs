...
public class RangeWeaponClient : IClientWeaponLogic
{
    ...
    protected static int GetActionDelayMs(float animationSpeed, int baseMilliseconds)
    {
        return Math.Max(100, (int)(baseMilliseconds / Math.Max(0.1f, animationSpeed)));
    }
    protected int GetActionDelayMs(EntityPlayer player, string animationCode, float animationSpeed, int fallbackBaseMilliseconds, string? callbackCode = null)
    {
        int baseMilliseconds = fallbackBaseMilliseconds;

        if (!string.IsNullOrEmpty(animationCode))
        {
            AnimationsManager? animationsManager = Api.ModLoader.GetModSystem<CombatOverhaulAnimationsSystem>()?.PlayerAnimationsManager;
            CombatOverhaul.Animations.Animation? animation = animationsManager?.GetAnimation(animationCode, player, firstPerson: true);
            ...
            if (animation != null)
            {
                if (!string.IsNullOrEmpty(callbackCode))
                {
                    ...
                    foreach (CallbackFrame callbackFrame in animation.CallbackFrames.Where(frame => frame.Code == callbackCode))
                    {
                        baseMilliseconds = (int)(animation.TotalDuration.TotalMilliseconds * callbackFrame.DurationFraction);
                        callbackFound = true;
                        break;
                    }
                    ...
                    if (!callbackFound)
                    {
                        baseMilliseconds = (int)animation.TotalDuration.TotalMilliseconds;
                    }
                }
                else
                {
        ...
        return GetActionDelayMs(animationSpeed, baseMilliseconds);
    }
