    ...
    protected virtual ItemSlot? GetBulletSlot(EntityPlayer player)
    {
        ...
        if (bulletSlot == null)
        {
            player.WalkInventory(slot =>
            {
                ...
                if (slot.Itemstack.Item.GetCollectibleBehavior<ProjectileBehavior>(true) != null && WildcardUtil.Match(Stats.BulletWildcard, slot.Itemstack.Item.Code.ToString()))
                {
                    bulletSlot = slot;
                    return false;
                }
...
public class SlingServer : RangeWeaponServer
{
    ...
    public override bool Reload(IServerPlayer player, ItemSlot slot, ItemSlot? ammoSlot, ReloadPacket packet)
    {
        ...
        if (ammoSlot?.Itemstack?.Item != null && ammoSlot.Itemstack.Item.GetCollectibleBehavior<ProjectileBehavior>(true) != null && WildcardUtil.Match(Stats.BulletWildcard, ammoSlot.Itemstack.Item.Code.ToString()))
        {
