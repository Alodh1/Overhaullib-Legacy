...
public class MeleeWeaponBehavior : CollectibleBehavior, IHasMultipleWeaponLogicModes, IHasWeaponLogic, IHasRangedWeaponLogic, IHasDynamicMoveAnimations, IHasMeleeWeaponActions, IHasServerBlockCallback, ISetsRenderingOffset, IMouseWheelInput, IOnGameTick, IRestrictAction
{
    ...
    public MeleeWeaponBehavior(CollectibleObject collObj) : base(collObj)
    {
        Item = (collObj as Item) ?? throw new Exception("must be an item");
    }
    ...
    public override WorldInteraction?[]? GetHeldInteractionHelp(ItemSlot inSlot, ref EnumHandling handling)
    {
        ...
        WorldInteraction?[]? interactionHelp = [];
    ...
    {
        ClientApi = clientAPI;
        ClientModes = new(clientAPI, Item);
        MeleeWeaponStats Stats = Item.Attributes.AsObject<MeleeWeaponStats>();
    ...
    {
        ServerLogic = new(serverAPI, Item);
    }
}
