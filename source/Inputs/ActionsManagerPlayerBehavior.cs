...
{
    private ItemSlot MainHandItemSlot
    {
        get
        {
            ItemSlot? active = _player.ActiveHandItemSlot;
            if (active?.Itemstack != null) return active;

            ItemSlot? right = _player.RightHandItemSlot;
            if (right?.Itemstack != null) return right;

            return active ?? right ?? _player.LeftHandItemSlot;
        }
    }
    ...
    public ActionsManagerPlayerBehavior(Entity entity) : base(entity)
    {
        ...
        try
        {
            _configuredPlayerUid = clientApi.Settings.String["playeruid"];
        }
        catch
        {
            _configuredPlayerUid = null;
        }
        _player = player;
        _api = clientApi;
    ...
    private readonly string? _configuredPlayerUid;
    private bool _weaponsRegistered = false;
    ...
    {
        if (_weaponsRegistered) return;

        IEnumerable<CollectibleBehavior> allBehaviors = _api.World.Items
            .SelectMany(item => item.CollectibleBehaviors ?? Array.Empty<CollectibleBehavior>());
    ...
    private bool HandleActionEvent(ActionEventData eventData, int itemId, ActionEventCallbackDelegate callback)
    {
        ...
        if (mainHandId == itemId)
        {
            try
            {
                mainHandHandled = callback.Invoke(MainHandItemSlot, _player, ref _mainHandState, eventData, true, _directionController.CurrentDirection);
            }
            catch (Exception exception)
            {
                LoggerUtil.Error(_api, this, $"Error while handling action '{eventData.Action}' for item '{MainHandItemSlot.Itemstack?.Collectible?.Code}':\n{exception}");
            }
        }
    ...
    private bool HandleHotkeyEvent(Dictionary<int, List<HotkeyEventCallbackDelegate>> delegates, KeyCombination keyCombination)
    {
        ...
        if (delegates.TryGetValue(mainHandId, out List<HotkeyEventCallbackDelegate>? delegatesForItemInMainHand))
        {
            foreach (HotkeyEventCallbackDelegate hotkeyDelegate in delegatesForItemInMainHand)
            {
                try
                {
                    if (hotkeyDelegate.Invoke(MainHandItemSlot, _player, ref _mainHandState, keyCombination, true, _directionController.CurrentDirection))
                    {
                ...
                {
                    LoggerUtil.Error(_api, this, $"Error while handling hotkey '{keyCombination}' for item '{MainHandItemSlot.Itemstack?.Collectible?.Code}':\n{exception}");
                }
            }
        }
    ...
    {
        int mainHandId = MainHandItemSlot.Itemstack?.Item?.Id ?? -1;
        int mainHandSlotId = MainHandItemSlot.Inventory.GetSlotId(MainHandItemSlot);
        int offHandId = _player.LeftHandItemSlot.Itemstack?.Item?.Id ?? -1;
        long mainHandStackId = GeneralUtils.GetItemMark(MainHandItemSlot);
    ...
    private void ProcessMainHandItemChanged()
    {
        ...
        ItemStack? stack = MainHandItemSlot.Itemstack;
        ...
        _mainHandState = 0;
        weapon.ClientLogic?.OnSelected(MainHandItemSlot, _player, true, ref _mainHandState);
    ...
    private bool SetRenderDirectionCursorForMainHand()
    {
        ...
        {
            DirectionsConfiguration directionsType = weapon.ClientLogic.DirectionsType;
            _directionController.DirectionsConfiguration = directionsType;
            _directionRenderer.Show = directionsType != DirectionsConfiguration.None;
        }

        return configuration != _directionController.DirectionsConfiguration;
    }

    private void TryBecomeMainPlayer()
    {
        if (_mainPlayer) return;

        bool isConfiguredPlayer =
            _player.PlayerUID != null &&
            _configuredPlayerUid != null &&
            _player.PlayerUID == _configuredPlayerUid;

        bool isWorldMainPlayer = _api.World.Player?.Entity?.EntityId == entity.EntityId;

        if (!isConfiguredPlayer && !isWorldMainPlayer) return;
