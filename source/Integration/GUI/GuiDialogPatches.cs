...
internal static class GuiDialogPatches
{
    ...
    private static bool GuiDialogInventory_ComposeSurvivalInvDialog(GuiDialogInventory __instance)
    {
        ...
        IInventory? backPackInv = ResolveInventoryField(__instance, GuiDialogInventory_backPackInv, "backPackInv", "backpackInv", "BackPackInv", "BackpackInv");
        IInventory? craftingInv = ResolveInventoryField(__instance, GuiDialogInventory_craftingInv, "craftingInv", "CraftingInv");

        if (backPackInv == null || craftingInv == null)
        {
            try
            {
                List<string> candidates = [];
                foreach (FieldInfo field in __instance.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance))
                {
                    if (!typeof(IInventory).IsAssignableFrom(field.FieldType)) continue;
                    object? value = field.GetValue(__instance);
                    candidates.Add($"{field.Name}:{field.FieldType.Name}={(value == null ? "null" : "set")}");
                }
                Api.Logger.Warning($"[OverhaullibLegacyCompat] Inventory fields unresolved. backpack={(backPackInv == null ? "null" : "set")} crafting={(craftingInv == null ? "null" : "set")} candidates=[{string.Join(", ", candidates)}]");
            }
            catch (Exception ex)
            {
                Api.Logger.Warning($"[OverhaullibLegacyCompat] Inventory field diagnostics failed: {ex}");
            }
            return true;
        }
        ...
        survivalInvDialog.GetScrollbar("scrollbar").SetHeights(
            ...
            (float)_rows.Select(rows => (rows + 0.4) * (GuiElementPassiveItemSlot.unscaledSlotSize + GuiElementItemSlotGrid.unscaledSlotPadding)).Sum() - 12f
        );
    ...
    {
        if (!api.ModLoader.IsModEnabled("combatoverhaul") && !api.ModLoader.IsModEnabled("combatoverhaulfork")) return;
    ...
    private static void ComposeBackpackSlots(GuiComposer composer, GuiDialogInventory __instance, IInventory? backPackInv, ElementBounds fullGridBounds)
    {
        ...
        for (int slotIndex = 4; slotIndex < backPackInv.Count; slotIndex++)
        {
            ...
            if (TryGetBackpackCategory(slot, out string categoryCode, out float orderPriority) && categoryCode != "")
            {
                ...
                if (!categoryExists)
                {
                    specialSlots.Add((orderPriority, categoryCode, [slotIndex]));
                }
            }
            else
            {
        ...
        composer.AddItemSlotGridExcl(
            ...
            6,
            GetInvertedIndexes(generalSlots, backPackInv.Count),
            fullGridBounds,
            "slotgrid"
        );
    ...
    private static bool TryGetBackpackCategory(ItemSlot slot, out string categoryCode, out float orderPriority)
    {
        categoryCode = "";
        orderPriority = 1f;

        if (slot is IHasSlotBackpackCategory typed)
        {
            categoryCode = typed.BackpackCategoryCode ?? "";
            orderPriority = typed.OrderPriority;
            return categoryCode != "";
        }

        // Fallback #1: read category directly from slot instance.
        object? directCategoryValue = GetAnyMemberValue(slot, "BackpackCategoryCode");
        object? directOrderValue = GetAnyMemberValue(slot, "OrderPriority");
        string? directCategory = AsNonEmptyString(directCategoryValue);
        if (directCategory != null)
        {
            categoryCode = directCategory;
            orderPriority = ParseOrderPriority(directOrderValue, 1f);
            return true;
        }

        // Fallback for mixed assembly load contexts where interface identity differs.
        object? config = GetAnyMemberValue(slot, "Config");
        if (config == null)
        {
            return TryInferBackpackCategory(slot, out categoryCode, out orderPriority);
        }

        object? categoryValue =
            GetAnyMemberValue(config, "BackpackCategoryCode") ??
            GetAnyMemberValue(config, "SlotBackpackCategory");
        object? orderValue =
            GetAnyMemberValue(config, "OrderPriority") ??
            GetAnyMemberValue(config, "CategoryOrderPriority");

        string? category = AsNonEmptyString(categoryValue);
        if (category != null)
        {
            categoryCode = category;
            orderPriority = ParseOrderPriority(orderValue, 1f);
            return true;
        }
    ...
    private static IInventory? ResolveInventoryField(object instance, FieldInfo? primaryField, params string[] fallbackFieldNames)
    {
        if (primaryField != null)
        {
            object? value = primaryField.GetValue(instance);
            if (value is IInventory invFromPrimary) return invFromPrimary;
        }

        Type type = instance.GetType();
        foreach (string fieldName in fallbackFieldNames)
        {
            FieldInfo? fallback = type.GetField(fieldName, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
            if (fallback == null) continue;
            object? value = fallback.GetValue(instance);
            if (value is IInventory inv) return inv;
        }
    ...
    private static bool TryInferBackpackCategory(ItemSlot slot, out string categoryCode, out float orderPriority)
    {
        categoryCode = "";
        orderPriority = 1f;

        string slotTypeName = slot.GetType().Name.ToLowerInvariant();
        if (slotTypeName.Contains("takeout"))
        {
        ...
        string iconCode = (GetAnyMemberValue(slot, "BackgroundIcon")?.ToString() ?? "").ToLowerInvariant();
        if (iconCode == "")
        {
            return false;
        }

        if (iconCode.Contains("arrow") || iconCode.Contains("bolt") || iconCode.Contains("bullet") || iconCode.Contains("sling"))
        {
            categoryCode = "ammunition";
            orderPriority = 0.8f;
            return true;
        }

        if (iconCode.Contains("sword") || iconCode.Contains("bow") || iconCode.Contains("pistol") || iconCode.Contains("weapon"))
        {
        ...
        if (iconCode.Contains("spanningtool") || iconCode.Contains("tool") || iconCode.Contains("knife"))
        {
            categoryCode = "tools";
            orderPriority = 0.7f;
            return true;
        ...
        if (iconCode.Contains("gear") || iconCode.Contains("bag") || iconCode.Contains("flute"))
        {
    ...
    private static object? GetAnyMemberValue(object instance, string memberName)
    {
        Type type = instance.GetType();
        ...
        foreach (FieldInfo candidate in type.GetFields(flags))
        {
            if (string.Equals(candidate.Name, memberName, StringComparison.OrdinalIgnoreCase)
                || string.Equals(candidate.Name, $"<{memberName}>k__BackingField", StringComparison.OrdinalIgnoreCase)
    ...
    private static float ParseOrderPriority(object? value, float fallback)
    {
