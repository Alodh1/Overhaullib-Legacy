...
public class GenericDisplayBlockEntity : GenericDisplayProto
{
    ...
    public override string InventoryClassName => "display-generic";
    ...
    public virtual bool OnInteract(IPlayer byPlayer, BlockSelection blockSel)
    {
        ...
        if (CanDisplayCollectible(colObj))
        {
            if (TryPut(slot, blockSel, byPlayer))
            {
                Api.World.PlaySoundAt(new AssetLocation("sounds/player/build"), byPlayer.Entity, byPlayer, randomizePitch: true, 16f);
                return true;
            }
            return false;
        }
    ...
    protected virtual bool MatchesAcceptedWildcards(CollectibleObject collectible)
    {
        string[] wildcards = Block?.Attributes?["acceptedWildcards"]?.AsArray<string>() ?? Array.Empty<string>();
        if (wildcards.Length == 0 || collectible?.Code == null) return false;

        return WildcardUtil.Match(wildcards, collectible.Code.Path) || WildcardUtil.Match(wildcards, collectible.Code.ToString());
    }

    protected virtual bool MatchesAcceptedItemTags(CollectibleObject collectible)
    {
        string[] acceptedTags = Block?.Attributes?["acceptedItemTags"]?.AsArray<string>() ?? Array.Empty<string>();
        if (acceptedTags.Length == 0 || collectible?.Attributes == null) return false;

        string[] collectibleTags = collectible.Attributes["tags"]?.AsArray<string>() ?? Array.Empty<string>();
        return acceptedTags.Any(tag => collectibleTags.Contains(tag));
    }

    protected virtual bool MatchesWallmountFallback(CollectibleObject collectible)
    {
        string blockCode = Block?.Code?.Path ?? string.Empty;
        if (!blockCode.Contains("mount", StringComparison.OrdinalIgnoreCase)) return false;
        if (collectible?.Attributes == null) return false;

        string[] collectibleTags = collectible.Attributes["tags"]?.AsArray<string>() ?? Array.Empty<string>();
        string tool = collectible.Attributes["tool"]?.AsString() ?? string.Empty;

        if (blockCode.Contains("shield", StringComparison.OrdinalIgnoreCase))
        {
            return collectibleTags.Contains("shield") || string.Equals(tool, "shield", StringComparison.OrdinalIgnoreCase);
        }

        if (blockCode.Contains("crossbow", StringComparison.OrdinalIgnoreCase))
        {
            return collectibleTags.Contains("crossbow");
        }

        return collectibleTags.Contains("firearm") || collectibleTags.Contains("crossbow") || collectibleTags.Contains("shield");
    }

    protected virtual bool CanDisplayCollectible(CollectibleObject collectible)
    {
        if (collectible?.Attributes == null) return false;
        if (collectible.Attributes.KeyExists(AttributeTransformCode)) return true;
        if (MatchesAcceptedWildcards(collectible)) return true;
        if (MatchesAcceptedItemTags(collectible)) return true;
        if (MatchesWallmountFallback(collectible)) return true;
        return false;
    }
}
