...
{
    public ClientItemAnimator(WalkSpeedSupplierDelegate walkSpeedSupplier, Vintagestory.API.Common.Animation[] animations, Action<string> onAnimationStoppedListener = null, Action<AnimationSound> onShouldPlaySoundListener = null) : base(walkSpeedSupplier, animations, onAnimationStoppedListener, AdaptSoundListener(onShouldPlaySoundListener))
    {
    }

    public ClientItemAnimator(WalkSpeedSupplierDelegate walkSpeedSupplier, Vintagestory.API.Common.Animation[] animations, ShapeElement[] rootElements, Dictionary<int, AnimationJoint> jointsById, Action<string> onAnimationStoppedListener = null, Action<AnimationSound> onShouldPlaySoundListener = null) : base(walkSpeedSupplier, animations, rootElements, jointsById, onAnimationStoppedListener, AdaptSoundListener(onShouldPlaySoundListener))
    {
    }
    ...
    private static Action<string, AnimationSound>? AdaptSoundListener(Action<AnimationSound>? listener)
    {
        return listener == null ? null : (_, sound) => listener(sound);
    }
}
...
public sealed class AnimatableShape : ITexPositionSource, IDisposable
{
    ...
    public static AnimatableShape? Create(ICoreClientAPI api, string shapePath, Item item)
    {
        ...
        AssetLocation shapeLocation = AssetLocation.Create(shapePath, item.Code.Domain);
